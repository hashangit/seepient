...
│   └── 022-4-remediation/            # Work order: follow-up-sweep closure (022-4 — plan complete 2026-09-13, tasks next via /speckit-tasks; sequenced AFTER 022-3 Round 2; disjoint by disposition ledger)
│       ├── spec.md                   # FR-001–FR-012 across US0 red gates → US1 symlink plane → US2 global-lifetime gate → US3 test-integrity infra → US4 auth completion + perms → US5 supply-chain pins; SC-001–SC-006 incl. pass-9 re-run
│       ├── plan.md                   # P0 red gates → P1 symlinks → P2 lifetime ∥ auth/perms → P3 integrity + pins → P4 release truth; ~1d; risks (realpath TOCTOU, over-blocking, R2 ordering + pull-forward contingency, probe seam design)
│       ├── research.md               # AUTHORITATIVE disposition ledger pass-8 → owner (R2 vs 022-4 vs unchanged) + evidence E1–E7 + decisions D1–D6 (refuse-don't-reclassify; classify-and-open one path; OQ-E suppression; probes-made-real w/ D5 deletion fallback; R2 ordering)
│       ├── data-model.md             # Read-side canonicalization refusal + realpath classification, lifetime gate shapes, path-keyed key cache, 0700/0600 modes, mutation-probe harness contract, new journey shapes, 6 invariants
...
- **ACTIVE PLAN (plan complete 2026-09-13 — tasks next via /speckit-tasks; sequenced AFTER 022-3 Round 2 lands, same branch/tree)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/022-multi-tenant-isolation/022-4-remediation/plan.md`
  ...
  probe-confirmed **symlink exfiltration pair** (read-side final-symlink
  refusal mirroring the destination rule analyzers.ts:874-878 +
  classify-what-you-read on the realpath + executors/media opening the
  authorized canonical realpath + relative-input resolution against the
  workspace root not process.cwd() — R2's FR-035 classification-only fix
  still inspects the LINK NAME, probe-defeated by a benign name);
