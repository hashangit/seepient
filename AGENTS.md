...
│   └── 022-5-engine-remediation/     # Engine boundary remediation (022-5 — GREEN-LIT 2026-09-24; plan COMPLETE, all artifacts ready, tasks via /speckit-tasks next): read-plane identity binding (dev/ino on the pinned fd — kills the folder-swap P0; media seam + edit_file oracle + FIFO), inference env-key DEMOLITION (credentials only via pi-ai provider management, owner decision; discovery sources fail CREDENTIAL_REQUIRED on valueless keys; no-undefined-to-vendored-SDK invariant test), REAL mutation probes (production-seam neutralization, message-asserted red flips, dogfood gate) + CHANGELOG claims corrections, P2 guard-surface batch (auth key-cache revocation race, root:"/"∥"" strip, GLOBAL typed denial not throw, casSync principal binding, htmlToText input cap, worker crash/413, racer kill-on-settle); pi-sync auto-release = ACCEPTED RISK (owner 2026-09-24)
...
- **ACTIVE PLAN (GREEN-LIT 2026-09-24 — plan + all artifacts complete; tasks via /speckit-tasks next)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/022-multi-tenant-isolation/022-5-engine-remediation/plan.md`
  — Engine boundary remediation (022-5): closes the pass-10 engine
  register under the authoritative charter
  (`Architecture/security-posture.md`). US1 read-plane identity binding:
  authorize-time `dev`/`ino` recorded on the prepared action, executor
  fstats the pinned fd and denies `READ_IDENTITY_MISMATCH` — kills the
  parent-dir-swap P0 (probe-confirmed in pass-10); media parity; the
  edit_file prepare-time oracle; FIFO/abort. US2 inference env-key
  DEMOLITION per owner decision (credentials resolve ONLY via pi-ai
  provider management; env synthesis at provider-config-store.ts:483
  deleted; discovery sources fail CREDENTIAL_REQUIRED on valueless
  keys; no-undefined-to-vendored-SDK invariant test). US3 REAL mutation
  probes (injected GuardProbe seam — no env branches in production
  code; red flips assert the security failure message, not the exit
  code; dogfood gate proves a deleted guard turns the matrix red) +
  CHANGELOG corrections for the three false 022-4 R2 claims. US4 P2
  batch: key-cache revocation race, universal-root strip ("/" and "*"
  and ""), GLOBAL typed denial not throw, casSync principal binding,
  htmlToText input cap, worker crash/413, racer kill-on-settle. Owner
  decisions 2026-09-24 recorded in spec.md: pi-sync auto-release =
  accepted risk; consent = permanent privilege dial (no consent changes
  in this spec). Lands before or with 024 (both plans carry the
  strip-layer sequencing rule: 022-5 first, or 024 rebases P3).
...
- **SHIPPED (021-3 remediation complete, v0.7.0)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/021-stateless-sdk-workers/021-3-remediation/tasks.md`
  ...
  quota regression, amnesiac resume (history never reaches the model),
  413 undeliverable on real sockets, WS eviction/crash paths, IPv6
  SSRF misclassification (`::7f00:1`/`::ffff:0:7f00:1` pass as public,
  `fd00:ec2::254` not metadata-gated), key-hash heuristics, ownership
  auto-adopt → fail-closed, type-truth edges, mock-blind test audit,
...
- **SHIPPED (remediated via 021-3; v0.7.0)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/021-stateless-sdk-workers/021-2-review-remediation/plan.md`
  ...
  worker-callback credentials. All 27 tasks complete (T001–T027).
...
- **SHIPPED (Spec 017 complete, v0.6.0, branch `017-permission-tool-baseline`)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/017-permission-tool-baseline/plan.md`
  ...
  permission denials, then the three consent modes (ask-everything `manual` /
  edit-enabled `balanced` — currently phantom / autonomous) with risk-informed
  prompting, `/mode` + Shift+Tab live toggling, per-workspace persistence, and
  an always-allowed class for core tools in every mode; P3 deletes
...
- **SHIPPED (spec 019 complete, branch `019-exact-commit-enforcement`)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/019-exact-commit-enforcement/plan.md`
  ...
  with unchecked commit-file caps, analyzers.ts:324), `write_file` fails
  ...
  users whole (gate before edit conversion; SDK wiring before ambient-fallback
  deletion; helper before fallback deletion).
- **UPCOMING (plan + tasks complete — implementation next, branch `018-skill-injection-visibility`)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/018-skill-injection-visibility/plan.md`
  ...
  20-run chat, duplicated on re-invocation); activations record bounded stubs
  + structured metadata, bodies inject ephemerally per run at request
  assembly (after the run's opening user message, alphabetical, repeat
  activations collapse within a run); the catalog contract flips to per-run
  semantics (default context = mandatory skills only; prior activations are
  not in context; the model MUST re-activate at run start); mandatory skills
  inject every run, trusted only from user-global scope or explicit opt-in
  (project-sourced self-declarations ignored); prompt assembly is
  volatility-ordered for prefix caching (frozen head of identity, tools,
  catalog, mandatory bodies; separated env block; append-only history; tail
  injection; no second-precision clock in the head — fixes the
  `buildSystemInfoBlock()` `toLocaleString()` landmine; deterministic
  ordering incl. append-only gateway tools; cache hit/miss split recorded
  when providers report it); applied-skills attribution on all four surfaces
  (TUI dedicated block + per-response attribution line + context panel, CLI
  line + `--json`, SDK result field + step metadata, server payload + SSE),
  all derived from one persisted per-run injection record; session resume
...
- **ACTIVE PLAN**: `~/Documents/Obsidian/Seepient/Implementation-Specs/013-provider-management-tui/plan.md`
  ...
  contracts/ (provider-manager-api, model-manager-dock, setup-wizard,
  surface-parity).
- **UPCOMING (plan + tasks complete — implementation next, branch `016-location-model`)**: `~/Documents/Obsidian/Seepient/Implementation-Specs/016-location-model/plan.md`
  — Location model & storage contract (016): one `foundations/paths.ts`
  constructs every path and parses every `SEEPIENT_*` path env (architecture
  rule enforces); one atomic-write helper gives the whole repo a single temp
  shape (`.seepient-<8hex>.tmp`, fsync, unlink-on-failure, stale sweep,
  0600/0700, symlink rejection); a Domain output resolver classifies every
  generated file as explicit-path / artifact / byproduct (deterministic
  `.seepient/output/<kind>/` floor, closed kind list shared with 012);
  standalone mode works from `~/seepient_documents` as Seepient's own
  workspace (signal-based detection, home access with denied zones, grants
  workspace/global can never collide, global never a default); barrel index
  (`.seepient/barrel.json`, directory-as-authority reconciliation, ≤30-topic
  digest injected into standalone prompts — no LLM loop); sessions unify
  under `~/.seepient/sessions` with `session-index.json` (node:sqlite at
  BMI, DuckDB rejected); skills source-rank absolute (frontmatter priority
  within-source only) + generated-skill writer (`kind: generated`
  frontmatter, collision refusal, version+changelog, path-leak validator,
  no approval); `docs/getting-started/storage.md` (≤20 rows) bound to code
  by a docs-sync test. Phases P1 foundations → P2 workspace/standalone →
  P3 sessions → P4 outputs/barrel → P5 skills/docs; tasks T001–T051 in
  tasks.md (test-first, US1–US5 story gates).
