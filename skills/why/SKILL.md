...
Before spawning investigators, enumerate the MCP tools available in the current session. Hermes exposes MCP servers configured in `config.yaml` (native MCP client) as ordinary tools (named `mcp__<server>__<tool>`), so check the active toolset. If no MCP server is configured, source control via git and `gh` is still always available; flag the other six categories as gaps.
...
Launch all matching investigators in a single `delegate_task` batch so they run concurrently. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Don't ask one agent to cover multiple MCPs.

Subagent config (each): a leaf `delegate_task` child, spawned in the same batch. Model follows `delegation.model` in config.yaml (default: the parent's model). Hermes has no readonly/Ask sandbox for subagents, so investigators keep the session's toolset (including MCPs) and read-only behavior is enforced in the prompt itself: they query and read, never write. That's a posture, not a sandbox.
...
Spawn one synthesizer `delegate_task` child (leaf role; model follows delegation config). Its quality check spot-verifies citations, which can require MCP access: the child inherits the session's tools, so it can query the same MCPs. Enforce read-only posture in the prompt.
