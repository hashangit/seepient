...
* Trusted-host execution is registry-only: the ambient tool-registry fallback
  is deleted, custom `trustedHostTool` tools are wired by the SDK composition
  roots, and a `permissions.trustedHostAllowlist` setting (default
  `["use_skill"]`) replaces the blanket trusted-host exemption — denial
...
* **Default Deployment Ceiling Baseline**: Built-in brokered tools (`generate_image`, `tavily_search`, `send_email`, `send_feishu_message`, `send_dingtalk_message`, `send_wecom_message`) are now auto-granted in the default ceiling policy (`DEFAULT_DEPLOYMENT_CEILING_V2`) and local policy baseline.
* **Dynamic Communication Analyzers**: `send_email`, `send_feishu_message`, `send_dingtalk_message`, and `send_wecom_message` extract dynamic destinations from tool arguments and configuration (`config.smtpHost`, webhook endpoints) rather than failing closed with missing static grants.
...
* **Zero-Prompt Normal Actions**: `get_current_date_time`, `todo_write`, `todo_read`, and `generate_widget` emit normal-class actions with `none` operations and normal `model-egress`, allowing them to execute instantly without human approval prompts across all modes.
