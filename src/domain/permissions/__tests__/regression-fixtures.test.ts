 ...
 */
describe("P0 regression fixtures — current defects (legacy path)", () => {
  it("QS-0.1: default `moderate` auto-approves `edit` (outside-write vector) [LEGACY]", () => {
    // CURRENT DEFECT (legacy matrix path): write_file at moderate + edit never
    // reaches an enforcement boundary. The matrix returns "auto" for edit at
    // moderate. This fixture documents the broken behavior of the LEGACY path.
    const decision = checkToolPermission("moderate", "edit");
    expect(decision).toBe("auto");
  });

  it("QS-0.2: `autoConfirm`/`permissive` bypasses all permission logic [LEGACY]", () => {
    // CURRENT DEFECT (legacy matrix path): permissive auto-approves even destructive actions.
