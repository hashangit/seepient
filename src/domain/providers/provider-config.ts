/**
 * Seepient Core — Provider Configuration Types & Mutation
 *
...
import type { LLMProvider, ProviderConfig } from "../../foundations/contracts/llm.js";
...
/**
 * Resolves provider configuration from AppConfig and creates the provider in
 * one call — the supported way for UI/Transport surfaces to obtain a provider
 * without importing the LLM capability directly. Returns null when the
 * provider isn't configured.
 */
export async function createProviderFromApp(
  config: AppConfig,
  providerType: ProviderType,
  modelOverride?: string,
): Promise<{ provider: LLMProvider; model: string } | null> {
  const resolved = resolveProviderConfigFromApp(config, providerType);
  if (!resolved) return null;
  if (modelOverride) resolved.model = modelOverride;
  const llmProvider = await createProvider(resolved);
  return { provider: llmProvider, model: resolved.model };
}

// ── CLI-config bridge ────────────────────────────────────────────────

/**
 * Load CLI-style config (~/.seepient/setting.json + .seepient/setting.json + env
...
export async function saveConfig(configPath?: string): Promise<void> {
  ...
  const targetPath = configPath ?? path.join(os.homedir(), ".seepientrc.json");
